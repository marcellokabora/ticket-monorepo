...
- **Base path: `/booking`** - All routes and static assets are served under this path

### Static Assets

**CRITICAL**: This app uses a base path of `/booking` configured in `svelte.config.js`. All static asset paths MUST include this prefix.

- Static files are located in: `apps/booking/static/`
- **Asset path pattern**: `/booking/assets/[filename]`
- **Examples**:
  - QR Code: `/booking/assets/qr-code-placeholder.svg`
  - Payment Logos: `/booking/assets/payment-logos/[logo].svg`
  - Favicon: `/booking/favicon.svg`

**Never use relative paths** (`./assets/`) or paths without the base (`/assets/`) - these will break in production. Always prefix static asset paths with `/booking/`.
...
- **Use Tailwind CSS exclusively with inline utility classes** - no component-scoped `<style>` blocks
- **All design tokens are configured in `packages/ui/src/styles/tailwind.css`**
- Reference the complete token list in `packages/ui/src/styles/tokens.css` for:
  - Colors (primary, secondary, neutral, system, overlays)
  - Spacing & Layout (gap, padding, generic spacing)
  - Border Radius, Shadows, Sizes, Typography
- Use Tailwind's semantic color classes: `bg-primary-500`, `text-secondary-400`, `text-neutral-black`
- Use Tailwind's built-in utilities that map to design tokens: `rounded-lg`, `shadow-md`, `gap-4`
- Use `class:` directive for conditional Tailwind classes: `class:bg-primary-500={active}`
- Implement responsive design with Tailwind's mobile-first breakpoints (sm:, md:, lg:, xl:)

**Tailwind Sizing Guidelines:**

- **Prefer standard Tailwind utilities over arbitrary values** (e.g., `h-13` instead of `h-[52px]`)
- Tailwind uses a **4px base unit** for spacing and sizing (e.g., `h-13` = 52px, `w-9` = 36px)
- Common conversions: `h-5` = 20px, `h-10` = 40px, `h-13` = 52px, `h-16` = 64px
- Use decimal values for odd sizes: `h-5.5` = 22px, `w-7.25` = 29px, `w-12.5` = 50px
- **Only use arbitrary values** `[value]` when no standard utility exists (e.g., `text-[8px]` or non-4px-divisible values)
- This prevents Tailwind IntelliSense warnings and maintains consistency

**Examples:**

- `bg-primary-500 text-neutral-white hover:bg-primary-600`
- `p-6 gap-3 rounded-lg shadow-md`
- `text-secondary-400 font-semibold leading-7`
- `bg-system-alert text-neutral-white`
- `h-13 w-full` instead of `h-[52px] w-full`
...
**Source:** [GitHub Awesome Copilot - Svelte Instructions](https://github.com/github/awesome-copilot/blob/main/instructions/svelte.instructions.md)
